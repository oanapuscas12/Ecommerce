...
public class DocumentsController {
    ...
    public String documentsList(
        ...
        if (userService.isMerchant()) {
            ...
            model.addAttribute("merchants", documentsService.getMerchantsWithDocuments());
        } else {
    ...
    public String uploadDocument(Model model) {
        ...
        if (userService.isMerchant() || userService.isAdmin()) {
            ...
            if (userService.isAdmin()) {
                List<User> users = userService.getNonAdminUsersExcluding(currentUser.getId());
    ...
    public ResponseEntity<byte[]> previewDocument(@PathVariable("documentId") Long documentId) {
        ...
        if (!Files.exists(filePath) || !Files.isReadable(filePath)) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Document not found on disk or is not readable");
        }
